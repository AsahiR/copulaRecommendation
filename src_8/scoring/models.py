...
class ScoreModel(metaclass=ABCMeta):
    ...
    def make_log(self,**no_use):
        pass
    def get_score_type_list(self):
        return self.score_type_list

    @abstractmethod
    def train(self, training_data_t: pd.DataFrame, training_data_f: pd.DataFrame,user_id:str,**no_use):
        #receive no_use,not need to use
        raise NotImplementedError

    @abstractmethod
    def calc_ranking(self, all_items: pd.DataFrame,**no_use) -> dict:
        raise NotImplementedError
...
def create_weight_and_scoring_model_list(
    ...
    for i, chunk in enumerate(filtered_hotel_cluster):
        ...
        for score_type in score_type_list:
            ...
            marginal_score_model=marginal.factory_marg(marg_name,marg_option=marg_option)
            marginal_score_model.set_param(chunk,score_type=score_type)
...
def create_weight_and_scoring_model_list_no_copula(
        hotel_cluster: List[pd.DataFrame],
        marg_name: str,
        cop: str,
        score_type_list: List[str],
    ...
    for i, chunk in enumerate(filtered_hotel_cluster):
        ...
        for score_type in score_type_list:
            marginal_score_list = chunk[score_type]
            marginal_score_model=marginal.factory_marg(marg_name,marg_option=marg_option)
            marginal_score_model.set_param(training_data=chunk,score_type=score_type)
...
def create_cluster(df:pd.DataFrame, n_clusters: int,target_axis_list: List[str],train_id:int,user_id:int)->List[pd.DataFrame]:
    hotel_cluster = []
    path=share.CLUSTER_TOP+'/'+str(n_clusters)+'/'+util.get_user_train_id_path(user_id,train_id)
    if share.REUSE and os.path.isfile(path):
        ...
        with open(path) as fout:
            ...
            if n_clusters == 1:
                ...
                for i in df.index:
                    ...
                    hotel=df.loc[i]
                    line=util.get_line_from_series(str(n_clusters)+',0',hotel)
                    fout.write(line+'\n')
            else:
                ...
                for i, cluster_num in enumerate(pred):
                    ...
                    # id_ignore???
                    line=util.get_line_from_series(row,line)
                    fout.write(line+'\n')
...
class CopulaScoreModel(ScoreModel):
    def __init__(self,cop: str,marg_name:str, n_clusters: int,remapping:bool,marg_option:None):
        ...
        self.prod_axis = []
        self.option_name+='copula-'+self.cop+'_'+'cluster-'+str(self.n_clusters)
    ...
    def set_dest_dict(self):
        ...
        super().dest_dict()
        self.dest_dict['log_marg_param']=share.MARG_TOP+'/'+self.get_dir_name()
        self.dest_dict['pdf_and_cdf']=share.PDF_CDF_TOP+'/'+self.get_dir_name()
    def get_dir_name(self)->str:
        return get_model_name()+'/'+get_option_name()+'/'+self.marg_model.get_dir_name()
    def make_log(self,all_items:pd.DataFrame,**no_use):
    ...
    def log_pdf_and_cdf(self,all_items:pd.DataFrame):
        ...
        dest=dest_dict['pdf_and_cdf']+'/'+self.get_dir_name()+'/'+self.user_train_id_path
        util.init_file(dest)
    ...
    def log_marg_param(self):
        ...
        keys=copy.deepcopy(self.score_type_list).append('copula')
        for key in keys:
            header+=','+key
        header+='\n'
        line='left'
        for cluster,(w,model_dict) in enumerate(weight_and_score_model_list):
    ...
    def train(self,training_data_t: pd.DataFrame, training_data_f: pd.DataFrame,train_id:int,user_id:int,**no_use):
        self.train_id,self.user_id=train_id,user_id
        self.user_train_id_path=util.get_user_train_id_path(train_id,user_id)
        hotel_cluster = create_cluster(training_data_t,self.n_clusters,self.score_type_list,self.train_id,self.user_id)
        # 混合コピュラの構築
        self.weight_and_score_model_list = create_weight_and_scoring_model_list(hotel_cluster, self.marg_option, self.cop, self.score_type_list)#org
    def calc_ranking(self, all_items: pd.DataFrame,**no_use) -> dict:
        dict_list = []
        dict_list_not_prod = []
        dict_list_emp = []
        ...
        return {'prod': df_for_ranking.sort_values(by='score', ascending=False),
...
class CopulaScoreModelDimensionReducedByUsingKL(CopulaScoreModel):
    def __init__(self, attn:str,const_a:float, cop: str,marg_name: str, n_clusters: int,remapping:bool, tlr:str,tlr_limit:float,marg_option:None):
        ...
        self.option_name+='/'+tlr+'_'+str(tlr_limit)
    def make_log(self,all_items:pd.DataFrame,**no_use):
        super().make_log(all_items=all_items)
        self.log_axis()
    def set_dest_dict(self):
        super().set_dest_dict()
        self.dest_dict['log_axis']=share.KL_PROFILE_TOP+'/'+self.get_dir_name()
        self.dest_dict['all_items_marg_dict']=share.ALL_ITMES_MARG_TOP+'/'+self.get_dir_name()
    def log_axis(self):
        dest=dest_dict['log_axis']+'/'+self.user_train_id_path
        ...
        for column in [self.bound_dict,self.kl_dict,self.prod_axis,self.score_type_list,self.reduced_axis,self.tl_axis]:
            object_quotaion='"'
            line+=','+object_quotation+str(column)+object_quotaion
        line+='\n'
        util.init_file(dest)
        with open(dest,'wt') as fout:
            fout.write(header)
    ...
    def select_axis(self,all_items:pd.DataFrame,mapping_id:str ,training_data_t: pd.DataFrame, training_data_f: pd.DataFrame):
        if not self.mapping_id or not self.mapping_id == mapping_id:
            #self.mapping_id is not init or reset ,reset all_items_marg
            self.mapping=mapping_id
            all_items_marg_path=dest_dict['all_items_marg_dict']+'/'+mapping_id
            if os.path.isfile(all_items_marg_path):
                ...
                for score_type in share.DEAFAUT_SCORE_TYPE_LIST:
                    all_marg=marginal.factory_marg(self.marg_name,marg_option=self.marg_option)
                    all_marg.set_param(training_data=all_items,score_type=score_type)
                    self.all_items_marg_dict[score_type]=marg 
                with open(all_items_marg_path,'wb') as fout:
                    pickle.dump(self.all_items_marg_dict,fout)
                
        self.kl_dict = {}
        for score_type in self.score_type_list:
            self.marg.set_param(training_data=training_data_t,score_type=score_type)
            #ok???
            kl = util.kl_divergence_between_population_and_users(self.all_items_marg_dict[score_type],attn,score_type,self.marg)
        ...
        self.med = statistics.median(kl_values)
        ...
        OrderedDict['bound1']=bound1
        OrderedDict['bound2']=bound2
        
        # self.score_type sorted by attn
        axis=sorted(self.score_type_list,key=lambda x: float(self.kl_dict[x]),reverse=True)
        self.prod_axis=[x for x in axis if sefl.kl_dict[x] > bound_dict['bound2']]
        self.tlr_axis=[]
        if not self.tlr=share.I_TLR:
            if self.tlr==share.TLR_NUM_UPPER:
                #deepcopy???
                self.tl_axis=axis[0:int(self.tlr_limit)]
            elif self.tlr==share.TLR_OL:
                bound_dict['ol']=med+madn*self.tlr_limit
    ...
    def train(self, train_id:int,training_data_t: pd.DataFrame, training_data_f: pd.DataFrame,user_id:int,all_items:pd.DataFrame,mapping_id:str,**no_use):
        self.user_id,self.train_id=user_id,train_id
        self.user_train_id_path=util.get_user_train_id_path(train_id,user_id)
        self.select_axis(training_data_t, training_data_f,all_items,mapping_id)
        #mapping_id is new, self.mapping_id is old
        super().train(all_items=all_items, training_data_t=training_data_t, training_data_f=training_data_f,train_id=train_id,user_id=user_id)
    def calc_ranking(self,all_items:pd.DataFram,**no_use)->dict:
        #{method:ranking,...}
        ranking_dict=super().calc_ranking(all_items=all_items)
        if not self.tlr=share.I_TLR:
            for method,ranking in ranking_dict.items():
                #call by reference
                util.tlr_filter(ranking_dict,self.weight_and_score_model_list,[(1.0,self.all_items_marg_dict)],self.tlr_axis,all_items) 
        return ranking_dict
...
class LinearScoreModelUserPreference(ScoreModel):
    ...
    def train(self, training_data_t: pd.DataFrame, training_data_f: pd.DataFrame, user_id:int,**no_use):
...
class RBFSupportVectorMachineModel(ScoreModel):
    ...
    #TEMPLATE = '{label} qid:1 1:{x1} 2:{x2} 3:{x3} 4:{x4} 5:{x5} 6:{x6} 7:{x7} 8:{x8}'#uc
    TEMPLATE = '{label} qid:1 1:{x1} 2:{x2} 3:{x3} 4:{x4} 5:{x5} 6:{x6} 7:{x7} 8:{x8} 9:{x9}'#uc
    ...
    def train(self, training_data_t: pd.DataFrame, training_data_f: pd.DataFrame, user_id:float,**no_use):
    ...
    def create_training_file(self, training_data_t: pd.DataFrame, training_data_f: pd.DataFrame):
        ...
        for index, row in training_data_t.iterrows():
            """
            content += self.TEMPLATE.format(label=1,
                                            x1=row['chargeScore'],
                                            x2=row['distanceScore'],
                                            x3=row['serviceScore'],
                                            x4=row['locationScore'],
                                            x5=row['roomScore'],
                                            x6=row['bathScore'],
                                            x7=row['equipmentScore'],
                                            x8=row['mealScore'])
            """
            content += self.TEMPLATE.format(label=1,
                                            x1=row['chargeScore'],
        ...
        for index, row in training_data_f.iterrows():
            """
            content += self.TEMPLATE.format(label=0,
                                            x1=row['chargeScore'],
                                            x2=row['distanceScore'],
                                            x3=row['serviceScore'],
                                            x4=row['locationScore'],
